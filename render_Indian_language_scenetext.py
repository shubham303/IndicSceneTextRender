...
densityOptions = {'100', '150', '150', '150', '200', '200', '200', '250', '250', '300', '300', '200', '150', '200',
                  '300', '300', '300', '250', '250'}
...
for i in range(resume_word_index, numWords):
	...
	shadowBooleanOpt = random.sample(shadowBooleanOptions, 1)[0]
	...
	#### add shadow/border ######
	if shadowBooleanOpt:  # add shadow only if boolean option is set.
	...
	return_code = os.system(command.encode('utf-8'))
	if return_code == 0:
		...
		if fgOrBgBoolean == 0:
			...
			textBgBlendImageName = str(
				i) + '_textBgBlend.png'  # the image which is blend of natural image and the fg text layer
